...
import type { Route } from "./+types/links.$id.destroy";
...
export async function clientAction({
  ...
  if (!result.ok)
    return err({ message: result.error ?? "Failed to delete link" });
...
function useDestroyState() {
  ...
  return location.state as { link?: Link } | undefined;
}

export default function DestroyLinkModal() {
      ...
      <ModalContent>
        <fetcher.Form method="post">
          ...
          <ModalBody>
            ...
            <p>Are you sure you want to delete "{state.link.name}"?</p>
          </ModalBody>
          <ModalFooter>
